...
import { CommStatusBadge } from "@/features/admin/communications/communications-demo-page";
...
export const Route = createFileRoute("/admin/communications/$communicationId")({
  loader: async ({ params }) => {
    ...
    return { communicationId: params.communicationId, comm };
  },
  head: ({ loaderData }) => {
    if (!loaderData)
      return {
...
export function DemoCommunicationNotFoundView() {
  return (
...
function CommunicationDetailPage() {
  ...
  const production = "production" in loaderData && loaderData.production;
...
export function DemoCommunicationDetailPage({
  loaderData,
}: {
  loaderData: Extract<
    ReturnType<typeof Route.useLoaderData>,
    { communicationId: string; comm: Communication }
  >;
}) {
  ...
  const comm = session.overlay(loaderData.comm);
...
export function loadDemoCommunicationDetail(communicationId: string) {
  const comm = getCommunication(communicationId);
  if (!comm) throw notFound();
  return { communicationId, comm };
}
