...
async function loadDemoQueuePage(
  filters: VerificationQueueFilters = {},
): Promise<VerificationQueuePage> {
  const normalized = normalizeQueueFilters(filters);
  let items = await loadDemoCases();
  ...
  if (normalized.search) {
    ...
    items = items.filter(
      (item) =>
        ...
        item.roleOrProgram.toLowerCase().includes(search) ||
...
export function createVerificationReviewAdapter(
  ...
  if (config.adminDemoMode) {
    return {
      mode: "demo",
      async listCases() {
        return (await loadDemoQueuePage()).items;
      },
...
export function verificationQueuePageQueryOptions(
  ...
  if (config.adminDemoMode) {
    return queryOptions({
      queryKey: verificationReviewKeys.page("demo", filters),
      queryFn: () => loadDemoQueuePage(filters),
    });
  }
...
function adaptDemoDetail(detail: VerificationCaseDetail): VerificationCaseDetail {
  ...
  return {
    ...
    statusMeta: buildStatusMeta(status, detail.summary.verificationType),
  };
}
